...
string OpenapiClient::invoke(const string &method, const string &content, const StringMap &extendParamMap) {
    ...
    string responseBody = getResponseBody(responseStr);
    string responseContent = analyzeResponse(responseBody);
...
string OpenapiClient::invokeGetHtml(const string &method, const JsonMap &contentMap, const StringMap &extendParamMap)
{
    string content = JsonUtil::objectToString(JsonType(contentMap));
       time_t t = time(0);
    char tmp[64];
    ...
    HttpClient httpClient;
    string responseStr = httpClient.sendSyncRequest(url, requestPairs);
    string htmlUrl = getHtmlurl(responseStr);
...
string OpenapiClient::getResponseBody(const string &responseStr){
       size_t pos = responseStr.find("\r\n\r\n");
        ...
        return responseStr.substr(pos + 2);
    }
    return responseStr.substr(pos + 4); // +4 for \r\n\r\n
}

string OpenapiClient::getHtmlurl(const string &response)
{
    std::string headerLine = "location:";
    size_t pos = response.find(headerLine);
    if (pos == std::string::npos) {
        return ""; 
    }
...
string OpenapiClient::rsaSign(const string &content, const string &key) {
    ...
    if (p_rsa != NULL) {
        ...
        {
            std::cerr<<"rsaSign error"<<std::endl;
        }
    }
...
string OpenapiClient::urlDecode(const std::string& str) {
    ...
    for (size_t i = 0; i < str.size(); ++i) {
        if (str[i] == '%' && i + 2 < str.size()) {
            unsigned char high = from_hex(str[i + 1]);
            unsigned char low = from_hex(str[i + 2]);
...
StringMap OpenapiClient::parseNotifyParams(const std::string& responseBody) {
    StringMap params;
    size_t start = 0;
    while (start < responseBody.length()) {
        size_t eq = responseBody.find('=', start);
        size_t amp = responseBody.find('&', start);
        if (eq == std::string::npos) break;

        std::string key = responseBody.substr(start, eq - start);
        std::string value = (amp == std::string::npos) 
            ? responseBody.substr(eq + 1) 
            : responseBody.substr(eq + 1, amp - eq - 1);

        params[urlDecode(key)] = urlDecode(value);
        start = (amp == std::string::npos) ? responseBody.length() : amp + 1;
    }
    return params;
}

StringMap OpenapiClient::getNotifyResponse(const string &responseBody)
{
    StringMap responseMap = parseNotifyParams(responseBody);
    string responseSign = responseMap["sign"];
    responseMap.erase("sign");
    responseMap.erase("sign_type");
    string signContent = buildContent(responseMap);

    // std::cout<<signContent<<std::endl;

    // 签名
     if (!alipayPublicKey.empty()) {
        //调用验签方法;
        bool verifyResult = OpenapiClient::rsaVerify(signContent, responseSign, alipayPublicKey);
        if (!verifyResult) {
            std::cerr<<("getNotifyResponse Verify Failed")<<std::endl;
            return StringMap();
        }
        DebugLog("Verify Success");
    } 
    else {
        std::cerr<<("AlipayPublicKey is empty, Skip the Verify")<<std::endl;
    }
